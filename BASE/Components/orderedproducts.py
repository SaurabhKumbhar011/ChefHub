...
class OrderedProducts(tk.Frame):
    def __init__(self, parent, root_frame, label_frame, table_num, func):
        ...
        self.tr_view = ttk.Treeview(self.tb, columns=self.tr_view_columns, show='headings', height=10,
                                    selectmode='browse')
    ...
    def populate_menu(self):
        retrieve_query = """SELECT id, table_num, product_name,  SUM(order_quantity) as order_quantity, order_status 
            FROM orders
            WHERE table_num = ? 
    ...
    def store_cooked_orders(self):
        try:
            ...
            for item in self.tr_view.get_children():
                ...
                self.fac_db.insert_spec_config(spec_insert_query,
                                               (order_id, self.t_num, or_name, or_quantity, or_total))
        except Error as e:
            print(e)
